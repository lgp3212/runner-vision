...
@observe()
def get_street_closures(
    lat: float, lng: float, radius_km: float = 0.5, days_back: int = 14
):
    """
        ...
        for closure in closures:
            try:
                ...
                for coord in all_coords:
                    ...
                    if (
                        abs(coord_lat - lat) <= lat_buffer
                        and abs(coord_lng - lng) <= lng_buffer
                    ):
                        is_nearby = True
...
def assess_closure_impact(closures_data: dict) -> dict:
    ...
    if total_closures == 0:
        return {"impact": "none", "message": "No active street closures in the area"}
...
if __name__ == "__main__":
    ...
    if closures.get("total_closures", 0) > 0:
        ...
        for i, closure in enumerate(closures["closures"][:3], 1):
            ...
            print(
                f"   Dates: {closure['work_start_date']} to {closure.get('work_end_date', 'ongoing')}"
            )
