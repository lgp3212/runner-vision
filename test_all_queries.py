...
def test_all_queries(
    start_lat, start_lng, target_distance_km, run_safety=True, run_closures=True
):
    """
    ...
    if run_safety or run_closures:
        ...
        for i, route in enumerate(routes, 1):
            ...
            enhanced_route = psa.analyze_route_comprehensive(
                route, check_safety=run_safety, check_closures=run_closures
            )
            ...
            if run_safety:
                ...
                dangerous_count = len(
                    enhanced_route["safety_analysis"]["dangerous_segments"]
                )
                print(
                    f"   ✓ Safety: {safety_score:.1f}/100, Dangerous segments: {dangerous_count}"
                )
            ...
            if run_closures:
                ...
                if enhanced_route["closure_analysis"]["closures"]:
                    all_closures_combined.extend(
                        enhanced_route["closure_analysis"]["closures"]
                    )
    ...
    print(
        f"✓ Weather: {weather.get('description', 'unknown')}, {weather.get('temperature_f', 0):.0f}°F"
    )
